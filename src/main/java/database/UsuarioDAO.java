...
// Consultas
    ...
    private void initStatement() throws SQLException {
        ...
        this.selectAutentica = con.getConnection().prepareStatement("select * from usuario where email = ? and senha = ?;");
        this.select = con.getConnection().prepareStatement("select * from usuario order by email;");
        this.delete = con.getConnection().prepareStatement("delete from usuario where id = ?");
        this.update = con.getConnection().prepareStatement("update usuario set email = ?, senha = ? where id = ?;");
        this.insert = con.getConnection().prepareStatement("insert into usuario (email, senha) values (?,?);");
    }
    ...
    public boolean autentica(String email, String senha) throws SQLException {
        this.selectAutentica.setString(1, email);
        this.selectAutentica.setString(2, senha);
    ...
    public ArrayList<Usuario> select() throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                todos.add(new Usuario(rs.getInt("id"), rs.getString("email"), rs.getString("senha")));
            }
            this.select.close();
    ...
    public boolean insert(Usuario usuario) {
        try {
            this.insert.setString(1, usuario.getEmail());
    ...
    public boolean update(Usuario usuario) {
        try {
            this.update.setString(1, usuario.getEmail());
